...
/**
 * 汇率工具类-rrexchanger /只提供非小号排名前100的币
 */
public class ConvertFXH {

  static String rrexchangerUrl = "https://dncapi.bqiapp.com/api/coin/web-coinrank?page=1&type=-1&pagesize=100&webp=1";
  ...
  public static BigDecimal getPriceByBtc(String coin) {
    return doUrl(rrexchangerUrl, coin, CurrencyFXH.BTC);
  }

  public static BigDecimal getPriceByUsd(String coin) {
    return doUrl(rrexchangerUrl, coin, CurrencyFXH.USD);
  }

  public static BigDecimal getPriceByCny(String coin) {
    return doUrl(rrexchangerUrl, coin, CurrencyFXH.CNY);
  }
  ...
  private static BigDecimal doUrl(String url, String symbol, CurrencyFXH currency) {
    ...
    try {
      if(dataCache.isEffeFXH()){
        result = dealData(dataCache.getrrexchanger().getJsonArray(),symbol,currency);
      }else{
        ...
        dataCache.setrrexchanger(com.github.oceanbbbbbb.bean.rrexchanger.builder().ctime(System.currentTimeMillis()).jsonArray(array).build());
